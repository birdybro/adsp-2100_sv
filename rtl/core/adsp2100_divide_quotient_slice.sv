...
module adsp2100_divide_quotient_slice (
    input  logic        clk_i,
    input  logic        reset_i,
    ...
    assign divisor_valid = dreg_valid_q[alternate_bank_o][divisor_source_dreg_o];
    assign source_known_o = (
        boundary_valid_o
        && divisor_valid
        && af_valid_q[alternate_bank_o]
        && dreg_valid_q[alternate_bank_o][DREG_AY0]
        && astat_valid_mask_q[5]
    );
    assign result_known_o = source_known_o;
    assign known_execute_enable = boundary_valid_o && source_known_o;
    assign old_aq_o = astat_o[5];
    assign add_divisor_o = old_aq_o;
    // DIVQ uses the 16-bit ALU result; carry/borrow beyond bit 15 is discarded.
    assign alu_result_o = old_aq_o
        ? (af_o + divisor_data)
        : (af_o - divisor_data);
    assign new_aq_o = divisor_data[15] ^ alu_result_o[15];
    assign quotient_bit_o = !new_aq_o;
    assign af_result_o = {alu_result_o[14:0], ay0_data[15]};
    assign ay0_result_o = {ay0_data[14:0], quotient_bit_o};
    assign divisor_before_o = divisor_data;
    assign partial_remainder_before_o = af_o;
    ...
    adsp2100_register_file registers (
        ...
        .read_address_0_i(divisor_source_dreg_o),
        .read_address_1_i(DREG_AY0),
        .read_address_2_i(DREG_AY0),
        .read_data_0_o(divisor_data),
        .read_data_1_o(ay0_data),
        .read_data_2_o(unused_read_data),
    ...
    // Validity sidebands preserve authentic unknown reset state without
    // substituting deterministic FPGA initialization for architectural data.
