...
module adsp2100_direct_jump_slice (
    input  logic        clk_i,
    input  logic        reset_i,
    ...
    adsp2100_direct_jump_decode decode (
        .opcode_i(opcode_i),
    ...
    assign condition_true_o = (
        boundary_valid_o && raw_condition_true
    );
    ...
    // PC is the first architectural program-counter storage connected to an
    // instruction decoder. RESET's 0x0004 value is original-device sourced.
    always_ff @(posedge clk_i) begin
        if (reset_i) begin
            ...
            if (pc_setup_enable) begin
                ...
                pc_q <= condition_true_o ? address_o : sequential_pc;
            end
