...
module adsp2100_shifter_dm_decode (
    ...
    assign class_valid_o = (
        (opcode_i & 24'hfe0000) == 24'h120000
    );
    assign dag_select_o = class_valid_o ? opcode_i[16] : 1'b0;
    ...
    assign i_address_o = (
        class_valid_o ? {opcode_i[16], opcode_i[3:2]} : 3'h0
    );
    assign m_address_o = (
        class_valid_o ? {opcode_i[16], opcode_i[1:0]} : 3'h0
    );
