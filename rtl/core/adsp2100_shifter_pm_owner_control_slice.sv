...
// Bounded Type 13/cache architectural client attached to the shared native
// PM owner and original normal-operation BR/BG sequencing.
//
// Ordinary fetch and Type 5 remain raw descriptor inputs. A collision has no
// undocumented priority: all requests are rejected, while the Type 13 client
// retains its descriptor and retries at a later enabled state-8 boundary.
module adsp2100_shifter_pm_owner_control_slice (
    input  logic        clk_i,
    input  logic        reset_i,
    ...
    assign type13_request_presented_o = issue_boundary_o && core_pm_select;
    ...
    assign attachment_conflict_o = (
        (type13_request_accepted_o && !type13_request_presented_o)
        || (completion_event_o[2]
    ...
    adsp2100_program_owner_bus_control owner_control (
        ...
        .type13_address_i(core_pm_address),
        .type13_address_valid_i(core_pm_address_valid),
        .type13_data_access_i(core_pm_data_access),
        .type13_write_i(core_pm_write),
        .type13_write_data_i(core_pm_write_data),
        .type13_write_data_valid_i(core_pm_write_data_valid),
...
`ifndef SYNTHESIS
    always_comb begin
        ...
        assert (type13_request_accepted_o == request_accepted_o[2]);
        if (type13_retry_pending_o) begin
    ...
    always_ff @(posedge clk_i) begin
        if (!reset_i && completion_event_o[2]) begin
