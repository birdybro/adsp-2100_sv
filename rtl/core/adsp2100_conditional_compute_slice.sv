...
module adsp2100_conditional_compute_slice (
    input  logic        clk_i,
    input  logic        reset_i,
    ...
    assign boundary_valid_o = (
        !reset_i && execute_i && action_valid_o && (setup_count == 4'h0)
    );
    ...
    assign alu_write_o = (
        boundary_valid_o && condition_true_o && is_alu_o && !nop_action_o
    );
    assign mac_write_o = (
        boundary_valid_o && condition_true_o && is_mac_o && !nop_action_o
    );
    ...
    always_comb begin
        if (boundary_valid_o && condition_true_o && !nop_action_o) begin
            assert ((is_mac_o && mac_valid && !alu_valid)
                    || (is_alu_o && alu_valid && !mac_valid));
        end
        if (!boundary_valid_o || !condition_true_o || nop_action_o) begin
