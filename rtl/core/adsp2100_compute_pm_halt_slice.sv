...
// Bounded original Type 5/cache/native-PM owner with HALT attachment.
//
// A HALT recognized during the PM-data phase latches a late recovery request
// in the Type 5 owner. The data action commits once, the following state-8
...
module adsp2100_compute_pm_halt_slice (
    input  logic        clk_i,
    input  logic        reset_i,
    ...
    adsp2100_compute_pm_native_slice owner (
        .clk_i(clk_i),
        .reset_i(reset_i),
        .phase_i(phase_i),
        ...
        .af_setup_write_i(af_setup_write_i),
        .af_setup_data_i(af_setup_data_i),
        .mf_setup_write_i(mf_setup_write_i),
