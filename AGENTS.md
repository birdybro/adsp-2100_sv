...
bank, including AF/MF/SB and unit-specific ALU/MAC/shifter writeback. MSTAT
integration, stateful stacks, complete operand/decode connectivity,
multifunction legality and ordering, status writeback, and cycle integration
do not. The executable instruction model establishes exact-width state, reset
unknowns, deterministic traces, PM fetch transactions, and only the
hand-verified all-zero NOP. All other opcodes fail closed. Architectural
