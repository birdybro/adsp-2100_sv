...
| CNTR state and CE transitions | PASS, PARTIAL | 12 directed/random model tests plus 50,022 stateful RTL cycles; condition/flow/count-stack connectivity, conditional-CALL CE, and empty-pop effects remain |
| PC/count/loop stack storage | PASS, PARTIAL | 10 directed/random model tests plus 50,062 stateful RTL cycles; action connectivity and empty-pop effects remain |
...
| Formal harnesses | SYNTAX PASS, PROOFS NOT RUN | 12 condition/ALU/MAC/shifter/DAG/sequencer-flow/CNTR/sequencer-stack/register/status/status-stack/MSTAT-integration recipes lint; SymbiYosys unavailable |
...
The implemented foundation regression is `make test`: 166 distinct Python
...
regressions, 50,022 CNTR cycles, 58,307 stateful DREG cycles, and 50,120
complete-bank/writeback cycles, plus 50,287 status/control and 50,037
status-stack state-transition cycles, plus 50,062 PC/count/loop stack-storage
cycles and 50,112 MSTAT-consumer integration cycles. Targets for unavailable
