...
| PC/count/loop stack storage | PASS, PARTIAL | 10 directed/random model tests plus 50,062 stateful RTL cycles; action connectivity, CNTR validity/decrement, and empty-pop effects remain |
...
| Formal harnesses | SYNTAX PASS, PROOFS NOT RUN | condition/ALU/MAC/shifter/DAG/sequencer-flow/sequencer-stack/register/status/status-stack/MSTAT-integration recipes lint; SymbiYosys unavailable |
...
The implemented foundation regression is `make test`: 154 distinct Python
...
cycles, plus 50,062 PC/count/loop stack-storage cycles and 50,112
MSTAT-consumer integration cycles. Targets for unavailable or unimplemented
areas print `SKIP` and do not create false pass evidence.
