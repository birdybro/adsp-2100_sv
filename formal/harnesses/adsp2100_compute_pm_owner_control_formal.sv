...
module adsp2100_compute_pm_owner_control_formal (
    input logic clk,
    input logic reset,
    ...
    assign unused_observation = ^{
        ...
        probe_m_valid, probe_l_data, probe_l_valid, px, px_valid, af,
        af_valid, mf, mf_valid, mr, mr_valid, astat, astat_valid_mask,
    ...
    always_comb begin
        ...
        assert (!type5_retry || (transaction_active && !type5_accepted));
        assert (!(completion[1]
        ...
        if (issue_inhibit) assert (!type5_presented);
    ...
    always_ff @(posedge clk) begin
        ...
        if (past_valid && !$past(reset) && !reset
            && $past(type5_accepted && recovery_fetch)
        ...
        cover (completion[1]);
        cover (completion[1] && data_complete);
        cover (completion[1] && instruction_complete && recovery_fetch);
        cover (bus_request_recognized && transaction_active);
        cover (resume && type5_accepted);
    end
endmodule
