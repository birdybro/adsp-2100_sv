...
module adsp2100_divide_quotient_formal (
    input logic        clk,
    input logic        reset,
    input logic        execute,
    ...
    assign expected_class = (opcode & 24'hfff8ff) == 24'h071000;
    assign expected_alu = old_aq
        ? (partial_before + divisor_before)
        : (partial_before - divisor_before);

    adsp2100_divide_quotient_slice dut (
        .clk_i(clk),
        .reset_i(reset),
        .execute_i(execute),
        .opcode_i(opcode),
        ...
        .old_aq_o(old_aq),
        .add_divisor_o(add_divisor),
        .new_aq_o(new_aq),
        .quotient_bit_o(quotient_bit),
        .divisor_before_o(divisor_before),
        .partial_remainder_before_o(partial_before),
    ...
    always_comb begin
        ...
        assert (
            boundary_valid
            == (!reset && execute && expected_class && setup_count == 3'd0)
        );
        ...
        unique case (xop)
            3'd0: assert (divisor_source == 4'h0);
            3'd1: assert (divisor_source == 4'h1);
            3'd2: assert (divisor_source == 4'ha);
            3'd3: assert (divisor_source == 4'hb);
            3'd4: assert (divisor_source == 4'hc);
            3'd5: assert (divisor_source == 4'hd);
            3'd6: assert (divisor_source == 4'he);
            default: assert (divisor_source == 4'hf);
        endcase
        if (source_known) begin
            assert (add_divisor == old_aq);
            assert (alu_result == expected_alu);
            assert (new_aq == (divisor_before[15] ^ alu_result[15]));
            assert (quotient_bit == !new_aq);
            assert (af_result == {alu_result[14:0], ay0_before[15]});
            assert (ay0_result == {ay0_before[14:0], quotient_bit});
        end

        cover (boundary_valid && source_known && old_aq);
        cover (boundary_valid && source_known && !old_aq);
        cover (boundary_valid && source_known && new_aq);
        cover (boundary_valid && source_known && !new_aq);
