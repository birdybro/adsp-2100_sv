...
module adsp2100_shift_move_formal (
    input logic        clk,
    input logic        reset,
    input logic        execute,
    ...
    function automatic logic [15:0] move_readback(
        input logic [3:0] destination,
        input logic [15:0] data
    );
        if (destination == 4'h9 || destination == 4'hd) begin
            return {{8{data[7]}}, data[7:0]};
        end
        return data;
    endfunction

    assign expected_class = ((opcode & 24'hff0000) == 24'h100000);
    assign expected_unused_x = expected_class && opcode[15];
    assign expected_unavailable_xop = (
        expected_class && !opcode[15] && opcode[10:8] == 3'b001
    );
    ...
    always_comb begin
        case (opcode[10:8])
            3'd0: expected_shifter_source = 4'h8;
            3'd2: expected_shifter_source = 4'ha;
            3'd3: expected_shifter_source = 4'hb;
            3'd4: expected_shifter_source = 4'hc;
            3'd5: expected_shifter_source = 4'hd;
            3'd6: expected_shifter_source = 4'he;
            3'd7: expected_shifter_source = 4'hf;
            default: expected_shifter_source = 4'h0;
        endcase
    end
    ...
    always_comb begin
        ...
        assert (unverified_unused_x == expected_unused_x);
        assert (unavailable_xop == expected_unavailable_xop);
        assert (destination_collision == expected_collision);
        assert (
            unsupported_subencoding == (expected_class && !expected_action)
        );
        assert (
            boundary_valid
        ...
        assert (
            shifter_source_dreg
            == (expected_action ? expected_shifter_source : 4'h0)
        );
        assert (
            move_destination_dreg
            == (expected_class ? opcode[7:4] : 4'h0)
        );
        ...
        if (probe_code == shifter_source_dreg) begin
            assert (probe_data == shifter_source_data);
        end
        assert (!(sr_write && (se_write || sb_write)));
        assert (!(se_write && sb_write));
        assert (!(ss_write && !(se_write && (sf == 4'hc || sf == 4'hd))));
        if (boundary_valid && sf <= 4'hb) begin
            assert (sr_write && !(se_write || sb_write || ss_write));
            assert (
                move_destination_dreg != 4'he
                && move_destination_dreg != 4'hf
            );
        end
        if (boundary_valid && sf >= 4'hc && sf <= 4'he) begin
            assert (move_destination_dreg != 4'h9);
        end
        if (boundary_valid && (sf == 4'hc || sf == 4'hd)) begin
            assert (se_write && ss_write && !(sr_write || sb_write));
        end
        cover (boundary_valid && move_source_dreg == shifter_source_dreg);
        cover (boundary_valid && sf == 4'h8 && sr_write);
        cover (boundary_valid && sf == 4'hd && se_write && ss_write);
        cover (boundary_valid && sf == 4'he && se_write && !ss_write);
        cover (boundary_valid && sf == 4'hf && sb_write);
        cover (unverified_unused_x);
        cover (unavailable_xop);
    ...
    always_ff @(posedge clk) begin
        if (!past_valid) begin
            ...
            if (probe_code == $past(move_destination_dreg)) begin
                assert (
                    probe_data
                    == move_readback(
                        $past(move_destination_dreg),
                        $past(move_source_data)
                    )
                );
            end
            if ($past(sr_write)) begin
                assert (sr == $past(sr_result));
            end
            if ($past(se_write)) begin
                assert (se == $past(se_result));
            end
            if ($past(sb_write)) begin
