...
module adsp2100_indirect_jump_formal (
    input logic        clk,
    input logic        reset,
    input logic        execute,
    ...
    input logic        i_setup,
    input logic [1:0]  i_setup_local,
    input logic [13:0] i_setup_data,
    input logic [1:0]  i_probe_local
);
    ...
    assign expected_class = ((opcode & 24'hffff20) == 24'h0b0000);
    ...
    always_comb begin
        ...
        assert (call_action == (expected_class ? opcode[4] : 1'b0));
        assert (i_local == (expected_class ? opcode[7:6] : 2'b00));
        assert (i_address == (expected_class ? {1'b1, opcode[7:6]} : 3'b000));
        ...
        assert (condition_known == (boundary_valid || invalid_target_state));
        assert (!(boundary_valid && invalid_target_state));
        assert (
        ...
        assert (pma_indirect_drive == condition_true);
        assert (!pma_indirect_drive || indirect_address_valid);
        ...
        cover (boundary_valid && !condition_true && !indirect_address_valid);
        cover (invalid_target_state);
        cover (pc_stack_push_accepted);
        cover (counter_decrement);
        cover (counter_restore);
        cover (unsupported_call_ce);
        cover (i_probe_valid && i_probe_data == 14'h1555);
    ...
    always_ff @(posedge clk) begin
        if (!past_valid) begin
            ...
            assert (!i_probe_valid);
        end else if ($past(boundary_valid)) begin
            assert (
                pc
                == (
                    $past(condition_true)
                    ? $past(indirect_address)
                    : ($past(pc) + 14'h0001)
                )
            );
        end else if ($past(
            !execute && pc_setup && !astat_setup
            && !counter_setup && !i_setup
        )) begin
        ...
        if ($past(
            !reset && !execute && i_setup && !pc_setup
            && !astat_setup && !counter_setup
            && i_setup_local == i_probe_local
        )) begin
            assert (i_probe_valid);
            assert (i_probe_data == $past(i_setup_data));
        end
        past_valid <= 1'b1;
    end
endmodule
