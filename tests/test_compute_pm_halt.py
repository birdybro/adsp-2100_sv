...
def _cycle(
    ...
    **kwargs: object,
    ...
    return apply_compute_pm_halt_cycle(state, phase=phase, **kwargs).state


def _setup_state(*, cache_address: int | None = None) -> ComputePMHaltState:
    state = ComputePMHaltState.reset()
...
class ComputePMHaltTests(unittest.TestCase):
    def test_pm_data_halt_overrides_hit_and_stops_after_real_fetch(self) -> None:
        ...
        recognized = apply_compute_pm_halt_cycle(
            state,
        ...
        for phase in (
            ...
            state = _cycle(state, phase, halt_n=False)
        data_done = apply_compute_pm_halt_cycle(
            state,
        ...
        self.assertEqual(
            read_dreg(data_done.state.native.core.core.primary, DREG.AR),
            ExactWord(16, 5),
        )

        forced = apply_compute_pm_halt_cycle(
            data_done.state,
            phase=LogicalPhase.STATE_8,
            halt_n=False,
        )
        ...
        for phase in (
            ...
            state = _cycle(state, phase, halt_n=False)
        stopped = apply_compute_pm_halt_cycle(
            state,
            phase=LogicalPhase.STATE_7,
            halt_n=False,
    ...
    def test_halted_state_holds_forced_fetch_bus_until_dmack_release(self) -> None:
        ...
        blocked = apply_compute_pm_halt_cycle(
            state,
            phase=LogicalPhase.STATE_8,
            halt_n=True,
            dmack=False,
        ...
        resumed = apply_compute_pm_halt_cycle(
            blocked.state,
            phase=LogicalPhase.STATE_8,
            halt_n=True,
            dmack=True,
    ...
    def test_compute_read_and_dag_commit_only_once_across_forced_fetch(self) -> None:
        ...
        self.assertEqual(
            read_dreg(state.native.core.core.primary, DREG.AR),
            ExactWord(16, 5),
        )
        self.assertEqual(
            read_dreg(state.native.core.core.primary, DREG.AX0),
            ExactWord(16, 0xCAFE),
        )
    ...
    def test_idle_halt_is_explicitly_outside_this_owner(self) -> None:
        result = apply_compute_pm_halt_cycle(
            ComputePMHaltState.reset(),
            phase=LogicalPhase.STATE_3,
            halt_n=False,
        )
    ...
    def test_reset_clears_native_and_control_state(self) -> None:
        state = ComputePMHaltState(
            ...
            control=apply_compute_pm_halt_cycle(
                ComputePMHaltState.reset(),
                phase=LogicalPhase.STATE_3,
                halt_n=False,
            ).state.control,
        ...
        self.assertEqual(result.state, ComputePMHaltState.reset())
