...
def _cycle(
    ...
    **kwargs: object,
    ...
    return apply_shifter_pm_owner_control_cycle(
        state, phase=phase, **kwargs
    ).state


def _setup_state() -> ShifterPMOwnerControlState:
...
class ShifterPMOwnerControlTests(unittest.TestCase):
    def test_machine_readable_attachment_contract(self) -> None:
        contract = json.loads(
            (ROOT / "docs/generated/adsp2100_shifter_pm_owner_control.yaml")
    ...
    def test_completed_fetch_populates_type13_cache(self) -> None:
        state = _setup_state()
        issued = apply_shifter_pm_owner_control_cycle(
            state,
        ...
        done = apply_shifter_pm_owner_control_cycle(
        ...
        type13 = apply_shifter_pm_owner_control_cycle(
            done.state,
            phase=LogicalPhase.STATE_8,
            execute=True,
        ...
        self.assertTrue(type13.core.core.accepted)
        self.assertTrue(type13.type13_request_accepted)
        self.assertEqual(
            type13.interface.owner_bus.accepted_owner,
            ProgramBusOwner.TYPE13_PM_DATA,
        )
        complete = apply_shifter_pm_owner_control_cycle(
            _advance_to_seven(type13.state),
            phase=LogicalPhase.STATE_7,
    ...
    def test_collision_retains_type13_descriptor_for_retry(self) -> None:
        issued = apply_shifter_pm_owner_control_cycle(
            _setup_state(),
            phase=LogicalPhase.STATE_8,
            execute=True,
        ...
        self.assertTrue(issued.type13_retry_pending)
        self.assertEqual(
            issued.interface.owner_bus.accepted_owner, ProgramBusOwner.NONE
        )
        self.assertIsNotNone(issued.state.core.core.pending)

        retry = apply_shifter_pm_owner_control_cycle(
            issued.state,
            phase=LogicalPhase.STATE_8,
        )
        self.assertTrue(retry.type13_request_presented)
        self.assertTrue(retry.type13_request_accepted)
        self.assertFalse(retry.type13_retry_pending)
        self.assertEqual(
    ...
    def test_recovery_collision_retries_without_replaying_data_action(
        ...
        data_done = apply_shifter_pm_owner_control_cycle(
            _advance_to_seven(issued.state),
            phase=LogicalPhase.STATE_7,
        ...
        recovered = apply_shifter_pm_owner_control_cycle(
    ...
    def test_br_completes_type13_then_masks_shared_pins(self) -> None:
        issued = apply_shifter_pm_owner_control_cycle(
            _setup_state(),
            phase=LogicalPhase.STATE_8,
            execute=True,
        ...
        recognized = apply_shifter_pm_owner_control_cycle(
        ...
        for phase in (
            ...
            state = _cycle(state, phase, br_n=False)
        granted = apply_shifter_pm_owner_control_cycle(
            state, phase=LogicalPhase.STATE_3, br_n=False
        )
        self.assertEqual(granted.interface.control.state.mode, BusControlMode.GRANTED)
        masked = apply_shifter_pm_owner_control_cycle(
    ...
    def test_raw_type5_completion_cannot_advance_type13(self) -> None:
        state = _setup_state()
        issued = apply_shifter_pm_owner_control_cycle(
            state,
            phase=LogicalPhase.STATE_8,
            type5_request=ProgramBusRequest.data_read(0x0777),
        )
        ...
        self.assertTrue(done.interface.owner_bus.type5_completion)
