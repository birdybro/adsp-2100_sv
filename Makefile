...
lint:
	...
	@if command -v "$(VERILATOR)" >/dev/null 2>&1; then \
		...
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
			--top-module adsp2100_program_owner_bus \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_program_bus.sv \
			rtl/core/adsp2100_program_owner_bus.sv; \
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
			--top-module adsp2100_program_owner_bus_control \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_program_bus.sv \
			rtl/core/adsp2100_program_owner_bus.sv \
		...
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
			--top-module adsp2100_bus_control \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_bus_control.sv; \
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
			--top-module adsp2100_reset_bus_grant \
			rtl/wrappers/adsp2100_reset_bus_grant.sv; \
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
...
pm-owner-bus-tests:
	$(PYTHON) -m unittest -v tests.test_program_owner_bus
	@if command -v "$(VERILATOR)" >/dev/null 2>&1; then \
		set -e; \
		$(PYTHON) tools/generators/generate_program_owner_bus_vectors.py \
			--output build/program_owner_bus_vectors.txt; \
		"$(VERILATOR)" --binary --timing --assert -Wall \
			-Wno-DECLFILENAME -Wno-TIMESCALEMOD \
			--Mdir build/obj_program_owner_bus \
			--top-module tb_adsp2100_program_owner_bus \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_program_bus.sv \
			rtl/core/adsp2100_program_owner_bus.sv \
			sim/unit/tb_adsp2100_program_owner_bus.sv; \
		build/obj_program_owner_bus/Vtb_adsp2100_program_owner_bus; \
	else \
		echo "SKIP shared-PM-owner RTL test: Verilator is not installed"; \
	fi

program-owner-bus-control-tests:
	$(PYTHON) -m unittest -v tests.test_program_owner_bus_control
	@if command -v "$(VERILATOR)" >/dev/null 2>&1; then \
		set -e; \
		$(PYTHON) tools/generators/generate_program_owner_bus_control_vectors.py \
			--output build/program_owner_bus_control_vectors.txt; \
		"$(VERILATOR)" --binary --timing --assert -Wall \
			-Wno-DECLFILENAME -Wno-TIMESCALEMOD \
			--Mdir build/obj_program_owner_bus_control \
			--top-module tb_adsp2100_program_owner_bus_control \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_program_bus.sv \
			rtl/core/adsp2100_program_owner_bus.sv \
			rtl/core/adsp2100_bus_control.sv \
...
compute-pm-owner-tests:
	$(PYTHON) -m unittest -v tests.test_compute_pm_owner_control
	@if command -v "$(VERILATOR)" >/dev/null 2>&1; then \
		set -e; \
		$(PYTHON) tools/generators/generate_compute_pm_owner_control_vectors.py \
			--output build/compute_pm_owner_control_vectors.txt; \
		"$(VERILATOR)" --binary --timing --assert -Wall \
			-Wno-DECLFILENAME -Wno-TIMESCALEMOD \
			--Mdir build/obj_compute_pm_owner_control_slice \
			--top-module tb_adsp2100_compute_pm_owner_control_slice \
			rtl/packages/adsp2100_pkg.sv \
			rtl/packages/adsp2100_register_pkg.sv \
			rtl/core/adsp2100_compute_pm_decode.sv \
			rtl/core/adsp2100_compute_dm_decode.sv \
		...
		build/obj_compute_pm_owner_control_slice/Vtb_adsp2100_compute_pm_owner_control_slice; \
	else \
...
bus-control-tests:
	$(PYTHON) -m unittest -v tests.test_bus_control
	@if command -v "$(VERILATOR)" >/dev/null 2>&1; then \
		set -e; \
		$(PYTHON) tools/generators/generate_bus_control_vectors.py \
			--output build/bus_control_vectors.txt; \
		"$(VERILATOR)" --binary --timing --assert -Wall \
			-Wno-DECLFILENAME -Wno-TIMESCALEMOD \
			--Mdir build/obj_bus_control \
			--top-module tb_adsp2100_bus_control \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_bus_control.sv \
			rtl/wrappers/adsp2100_reset_bus_grant.sv \
			sim/unit/tb_adsp2100_bus_control.sv; \
		build/obj_bus_control/Vtb_adsp2100_bus_control; \
	else \
		echo "SKIP BR/BG RTL test: Verilator is not installed"; \
	fi
...
linear-bus-control-tests:
	$(PYTHON) -m unittest -v tests.test_linear_bus_control
	@if command -v "$(VERILATOR)" >/dev/null 2>&1; then \
		set -e; \
		$(PYTHON) tools/generators/generate_linear_bus_control_vectors.py \
			--output build/linear_bus_control_vectors.txt; \
		"$(VERILATOR)" --binary --timing --assert -Wall \
			-Wno-DECLFILENAME -Wno-TIMESCALEMOD \
			--Mdir build/obj_linear_bus_control \
			--top-module tb_adsp2100_linear_bus_control_slice \
			rtl/packages/adsp2100_pkg.sv \
			rtl/packages/adsp2100_register_pkg.sv \
			rtl/core/adsp2100_load_dreg_immediate_decode.sv \
			rtl/core/adsp2100_load_non_dreg_immediate_decode.sv \
...
formal:
	@if command -v "$(VERILATOR)" >/dev/null 2>&1; then \
		...
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
			--top-module adsp2100_program_owner_bus_formal \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_program_bus.sv \
			rtl/core/adsp2100_program_owner_bus.sv \
			formal/harnesses/adsp2100_program_owner_bus_formal.sv; \
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
			--top-module adsp2100_program_owner_bus_control_formal \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_program_bus.sv \
			rtl/core/adsp2100_program_owner_bus.sv \
			rtl/core/adsp2100_bus_control.sv \
		...
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
			--top-module adsp2100_compute_pm_owner_control_formal \
			rtl/packages/adsp2100_pkg.sv \
			rtl/packages/adsp2100_register_pkg.sv \
			rtl/core/adsp2100_compute_pm_decode.sv \
			rtl/core/adsp2100_compute_dm_decode.sv \
		...
		"$(VERILATOR)" --lint-only --assert -Wall -Wno-DECLFILENAME \
			--top-module adsp2100_bus_control_formal \
			rtl/packages/adsp2100_pkg.sv \
			rtl/core/adsp2100_bus_control.sv \
			rtl/wrappers/adsp2100_reset_bus_grant.sv \
	...
	@if command -v sby >/dev/null 2>&1; then \
		...
		sby -f -d build/formal_program_owner_bus \
...
synth-yosys:
	@if command -v yosys >/dev/null 2>&1; then \
		...
		yosys -q -l build/yosys_program_owner_bus.log \
...
clean:
	...
	@find build -maxdepth 1 -type f -name program_owner_bus_vectors.txt -delete
	@find build -maxdepth 1 -type f -name yosys_program_owner_bus.log -delete
	@find build -maxdepth 1 -type f -name yosys_program_owner_bus.json -delete
	...
	@if [ -d build/obj_program_owner_bus ]; then \
		find build/obj_program_owner_bus -depth -delete; \
	fi
	...
	@if [ -d build/quartus_program_owner_bus ]; then \
		find build/quartus_program_owner_bus -depth -delete; \
	fi
