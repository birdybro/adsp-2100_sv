...
module tb_adsp2100_shifter_pm_decode;
    logic [23:0] opcode;
    logic class_valid;
    logic action_valid;
    logic unsupported;
    ...
    adsp2100_shifter_pm_decode dut (
        .opcode_i(opcode),
    ...
    initial begin
        ...
        for (word = 0; word < 16_777_216; word = word + 1) begin
            opcode = word[23:0];
            #1;
            expected_class = ((opcode & 24'hff0000) == 24'h110000);
            ...
            if (expected_class) begin
                if (write_direction !== opcode[15] || sf !== opcode[14:11]
                    || xop !== opcode[10:8] || memory_dreg !== opcode[7:4]
                    || i_address !== {1'b1, opcode[3:2]}
                    || m_address !== {1'b1, opcode[1:0]}) begin
                    $fatal(1, "field mismatch word=%06x", word);
                end
                class_count = class_count + 1;
            end else if (write_direction || sf != 0 || xop != 0
        ...
        if (class_count != 65_536 || action_count != 54_320
            || unavailable_count != 8_192 || collision_count != 3_024) begin
            $fatal(
                1,
        ...
        $display(
            "PASS Type 13 exhaustive decode: %0d supported, %0d unsupported",
            action_count,
