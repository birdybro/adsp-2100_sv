...
module tb_adsp2100_shifter_pm_halt_slice;
    logic         clk;
    logic [191:0] stimulus;
    ...
    assign {
        ...
        dreg_setup, dreg_setup_code, dreg_setup_data, sb_setup,
        sb_setup_data, dag_setup, dag_setup_kind, dag_setup_address,
    ...
    initial begin
        ...
        expected_events = '0;
        expected_post = '0;
        vector_file = $fopen("build/shifter_pm_halt_vectors.txt", "r");
        if (vector_file == 0) begin
            $fatal(1, "cannot open Type 13/native-PM/HALT vectors");
        end
        vector_count = 0;
        while (!$feof(vector_file)) begin
            ...
            if (scan_count == 3) begin
                ...
                if (actual_events !== expected_events) begin
                    $fatal(
                        1,
                        "Type 13/HALT event mismatch vector=%0d expected=%032x actual=%032x stimulus=%048x",
                        vector_count,
                        expected_events,
                        actual_events,
                        stimulus
                ...
                if (actual_post !== expected_post) begin
                    $fatal(
                        1,
                        "Type 13/HALT post mismatch vector=%0d expected=%040x actual=%040x",
                        vector_count,
                        expected_post,
                        actual_post
                    );
        ...
        if (vector_count < 50_000) begin
            $fatal(
                1,
                "insufficient Type 13/native-PM/HALT vectors: %0d",
                vector_count
            );
        end
        $display(
            "PASS Type 13/cache/native-PM/HALT differential: %0d clocks",
            vector_count
        );
        $finish;
    end
