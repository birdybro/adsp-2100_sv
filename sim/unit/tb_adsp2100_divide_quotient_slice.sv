...
module tb_adsp2100_divide_quotient_slice;
    logic         clk;
    logic [77:0]  stimulus;
    logic [120:0] expected_events;
    ...
    logic exp_old_aq;
    logic exp_add_divisor;
    logic exp_new_aq;
    logic exp_quotient_bit;
    logic exp_af_write;
    logic exp_ay0_write;
    logic exp_aq_write;
    ...
    logic exp_dm_access;
    logic [2:0] exp_xop;
    logic [3:0] exp_divisor_source;
    logic exp_compare_values;
    logic [15:0] exp_divisor_before;
    logic [15:0] exp_partial_before;
    ...
    initial begin
        ...
        expected_events = '0;
        expected_post = '0;
        vector_file = $fopen("build/divide_quotient_vectors.txt", "r");
        if (vector_file == 0) begin
            $fatal(1, "cannot open build/divide_quotient_vectors.txt");
        end
        vector_count = 0;
        while (!$feof(vector_file)) begin
            ...
            if (scan_count == 3) begin
                ...
                if (
                    ...
                    || xop !== exp_xop
                    || divisor_source !== exp_divisor_source
                ) begin
                    $fatal(1, "event/field mismatch vector=%0d", vector_count);
                end
                if (exp_compare_values) begin
                    if (
                        old_aq !== exp_old_aq
                        || add_divisor !== exp_add_divisor
                        || new_aq !== exp_new_aq
                        || quotient_bit !== exp_quotient_bit
                        || divisor_before !== exp_divisor_before
                        || partial_before !== exp_partial_before
        ...
        $display(
            "PASS Type 23 stateful model/RTL differential: %0d cycles",
            vector_count
        );
        $finish;
    end
