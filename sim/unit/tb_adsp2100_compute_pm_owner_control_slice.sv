...
module tb_adsp2100_compute_pm_owner_control_slice;
    logic clk;
    logic [235:0] stimulus;
    ...
    logic type13_valid, type13_address_valid, type13_write;
    logic [13:0] type13_address;
    logic [23:0] type13_data;
    logic type13_data_valid;
    logic execute;
    ...
    logic af_setup, mf_setup;
    logic [15:0] af_setup_data, mf_setup_data;
    logic dag_setup;
    ...
    logic cache_fill_accepted, type5_presented, type5_accepted;
    logic type5_retry;
    ...
    assign {
        ...
        type13_valid, type13_address, type13_address_valid, type13_write,
        type13_data, type13_data_valid,
    ...
    adsp2100_compute_pm_owner_control_slice dut (
        ...
        .px_o(px), .px_valid_o(px_valid), .af_o(af),
        .af_valid_o(af_valid), .mf_o(mf), .mf_valid_o(mf_valid),
        .mr_o(mr), .mr_valid_o(mr_valid), .astat_o(astat),
    ...
    initial begin
        ...
        expected_post = '0;
        vector_file = $fopen(
            "build/compute_pm_owner_control_vectors.txt", "r"
        );
        if (vector_file == 0)
            $fatal(1, "cannot open Type 5/shared-PM/BR-BG vectors");
        vector_count = 0;
        while (!$feof(vector_file)) begin
            ...
            if (scan_count == 3) begin
                ...
                if (actual_events !== expected_events)
                    $fatal(1, "Type 5/shared-PM pre mismatch vector=%0d expected=%041x actual=%041x stimulus=%059x",
                           vector_count, expected_events, actual_events,
                           stimulus);
                #2 clk = 1'b1;
                #1;
                if (actual_post !== expected_post)
                    $fatal(1, "Type 5/shared-PM post mismatch vector=%0d expected=%047x actual=%047x",
        ...
        $display(
            "PASS Type 5/shared-PM/BR-BG differential: %0d clocks",
            vector_count
        );
        $finish;
    end
