...
module tb_adsp2100_direct_jump_slice;
    logic        clk;
    logic [64:0] stimulus;
    logic [40:0] expected_events;
    logic [85:0] expected_state;
    logic        reset;
    logic        execute;
    logic [23:0] opcode;
    logic        pc_setup;
    ...
    initial begin
        ...
        expected_events = '0;
        expected_state = '0;
        vector_file = $fopen("build/direct_jump_vectors.txt", "r");
        if (vector_file == 0) begin
            $fatal(1, "cannot open Type 10 vectors");
        end
        vector_count = 0;
        while (!$feof(vector_file)) begin
            ...
            if (scan_count == 3) begin
                ...
                if ({
                    ...
                    call_action, address, condition,
                    boundary_valid, invalid_opcode, integration_conflict,
                    invalid_condition_state, condition_known, condition_true,
        ...
        $fclose(vector_file);
        if (vector_count != 554_412) begin
            $fatal(1, "unexpected vector count=%0d", vector_count);
        end
        $display(
            "PASS Type 10 bounded model/RTL differential: %0d cycles",
            vector_count
        );
        $finish;
    end
