...
module tb_adsp2100_compute_pm_decode;
    logic [23:0] opcode;
    logic        class_valid;
    logic        action_valid;
    ...
    adsp2100_compute_pm_decode dut (
        .opcode_i(opcode),
    ...
    initial begin
        ...
        for (word = 0; word < 16_777_216; word = word + 1) begin
            opcode = word[23:0];
            #1;
            expected_class = ((opcode & 24'hf00000) == 24'h500000);
            ...
            if (expected_class) begin
                if (write_direction !== opcode[19]
                    || destination_feedback !== opcode[18]
                    || amf !== opcode[17:13]
                    || yop !== opcode[12:11]
                    || xop !== opcode[10:8]
                    || memory_dreg !== opcode[7:4]
                    || i_address !== {1'b1, opcode[3:2]}
                    || m_address !== {1'b1, opcode[1:0]}) begin
                    $fatal(1, "field mismatch word=%06x", word);
                end
            end else if (write_direction !== 1'b0
        ...
        if (class_count != 1_048_576 || action_count != 1_017_344
            || collision_count != 31_232) begin
            $fatal(
                1,
        ...
        if (!action_valid || write_direction || amf != 5'h13
            || memory_dreg != 4'h0 || i_address != 3'h4
            || m_address != 3'h7) begin
            $fatal(1, "manual ALU/read fixture mismatch");
        end
        opcode = 24'h5a60ac;
        #1;
        if (!action_valid || !write_direction || amf != 5'h13
            || memory_dreg != 4'ha || i_address != 3'h7
            || m_address != 3'h4) begin
            $fatal(1, "manual ALU/write fixture mismatch");
        end
        $display(
            "PASS Type 5 decode: %0d supported, %0d collisions",
            action_count,
            collision_count
        );
        $finish;
