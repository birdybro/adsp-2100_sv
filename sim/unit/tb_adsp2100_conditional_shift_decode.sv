...
module tb_adsp2100_conditional_shift_decode;
    logic [23:0] opcode;
    logic        class_valid;
    logic        action_valid;
    ...
    adsp2100_conditional_shift_decode dut (
        .opcode_i(opcode),
    ...
    initial begin
        ...
        if (class_count != 2_048 || action_count != 1_792
            || unsupported_count != 256) begin
            $fatal(
                1,
        ...
        $display(
            "PASS Type 16 decode: %0d legal, %0d unsupported subencodings",
            action_count,
            unsupported_count
        );
        $finish;
