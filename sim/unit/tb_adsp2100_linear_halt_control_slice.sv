...
module tb_adsp2100_linear_halt_control_slice;
    ...
    initial begin
        ...
        expected_post = '0;
        vector_file = $fopen(
            "build/linear_halt_control_vectors.txt",
            "r"
        );
        if (vector_file == 0) begin
            $fatal(1, "cannot open linear-HALT vectors");
        end
        vector_count = 0;
        while (!$feof(vector_file)) begin
            ...
            if (scan_count == 4) begin
                ...
                if ({
                    ...
                    $fatal(
                        1,
                        "linear-HALT control mismatch vector=%0d got=%h expected=%h",
                        vector_count,
                        {
                            halt_mode, state_three_boundary,
                            halt_recognized, halt_stop_event,
                ...
                if ({
                    ...
                    $fatal(1, "linear-HALT core mismatch vector=%0d", vector_count);
                end
                ...
                if ({
                    ...
                    $fatal(1, "linear-HALT post mismatch vector=%0d", vector_count);
                end
        ...
        if (vector_count < 50_000) begin
            $fatal(1, "insufficient linear-HALT vectors: %0d", vector_count);
        end
        $display(
            "PASS bounded linear-fetch/HALT composition: %0d clocks",
            vector_count
        );
        $finish;
    end
