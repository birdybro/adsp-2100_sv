"""Composed original ADSP-2100 Type 13/cache transaction model.

The Type 13 data action and the instruction-cache monitor remain independent
models.  This boundary supplies the next-fetch cache lookup to Type 13 and
feeds every external recovery instruction back into the monitor.  An explicit
ordinary-fetch completion port is retained so non-Type-13 fetches can populate
the same cache before a program-memory data cycle.
"""
...
class ShifterPMCacheState:
    core: ShifterPMState = field(default_factory=ShifterPMState.reset)
...
@dataclass(frozen=True)
class ShifterPMCacheCycleResult:
    state: ShifterPMCacheState
    core: ShifterPMCycleResult
...
def apply_shifter_pm_cache_cycle(
    state: ShifterPMCacheState,
    *,
    reset: bool = False,
    execute: bool = False,
    ...
    """Apply one cache fill, Type 13 data cycle, or recovery fetch.

    A standalone external instruction fill has priority over a newly requested
    Type 13/setup action and reports a conflict.  An already scheduled recovery
    has priority over a simultaneous standalone fill because it owns the PM
    transaction for that fixed logical cycle.  This is a fail-closed harness
    policy for an integration error, not a claim about undocumented device
    arbitration; a complete issue controller must prevent either collision.
    """
    ...
    core_result = apply_shifter_pm_cycle(
        state.core,
        reset=reset,
        ...
        setup_sb=None if suppress_controls else setup_sb,
