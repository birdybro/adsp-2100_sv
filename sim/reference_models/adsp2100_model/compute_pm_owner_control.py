"""Type 5 architectural client attached to shared PM ownership and BR/BG.

The Type 5/cache model retains its own data or recovery descriptor.  This
composition presents that retained descriptor only at an enabled state-8
boundary, routes only the Type 5 owner's completion back to the client, and
uses completed ordinary fetches to fill the same instruction cache.  Raw
ordinary-fetch and Type 13 descriptors remain external bounded requesters.
"""
...
class ComputePMOwnerControlState:
    ...
    @classmethod
    def reset(cls) -> "ComputePMOwnerControlState":
        return cls()


@dataclass(frozen=True)
class ComputePMOwnerControlCycleResult:
...
def apply_compute_pm_owner_control_cycle(
    ...
    *,
    ...
    """Apply one bounded Type 5/shared-PM/BR-BG clock."""
    ...
    core = apply_compute_pm_cache_cycle(
        state.core,
        reset=reset,
    ...
    if issue_boundary and core.core.pm_select:
        type5_request = ProgramBusRequest(
            address=(
    ...
    type5_presented = type5_request is not None
    type5_accepted = interface.owner_bus.type5_accepted
    type5_retry = bool(type5_presented and not type5_accepted)
    attachment_conflict = bool(
        ...
        or preview.owner_bus.type5_completion
        and not (
    ...
    return ComputePMOwnerControlCycleResult(
        state=ComputePMOwnerControlState(core.state, interface.state),
        core=core,
        interface=interface,
        issue_boundary=issue_boundary,
        phase_conflict=phase_conflict,
        fetch_cache_fill=fetch_cache_fill,
        type5_request_presented=type5_presented,
        type5_request_accepted=type5_accepted,
        type5_retry_pending=type5_retry,
