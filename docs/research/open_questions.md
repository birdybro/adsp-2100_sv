...
| OQ-008 | What are the exact hidden cache-monitor transitions and PM-data miss rules for self-modifying PM, branch/loop boundaries, interrupts, HALT, and BR? | the sourced functional monitor uses one contiguous up-to-16-word region, low-four-bit indexing, out-of-region invalidation, and circular oldest replacement; the connected Type 13 boundary now selects cached data and fills recovery words, while hidden ahead/behind encodings and unified event arbitration remain | 1989 pp. 4-26–4-30 and 5-13–5-16, directed original simulator/hardware traces | cycle accuracy | PARTIALLY BOUNDED |
