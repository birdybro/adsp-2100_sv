#!/usr/bin/env python3
"""Generate deterministic Type 16 stateful model-versus-RTL vectors."""
...
def generate_lines(random_count: int, seed: int) -> list[str]:
    ...
    state = ConditionalShiftState.reset()
    lines: list[str] = []
    ...
    def emit(
        *,
        ...
        if result.boundary_valid and not result.condition_known:
            raise AssertionError("RTL vectors require a known Type 16 predicate")
        ...
        lines.append(f"{stimulus:018x} {events:08x} {post_state:022x}")
        state = result.state
    ...
    # Two status/counter patterns give both outcomes for every nonconstant
    # condition while all 1,792 supported actions execute in each bank.
    for bank in (0, 1):
        emit(setup_mstat=ExactWord(4, bank))
        for astat, not_counter_expired in ((0x00, False), (0x5D, True)):
                ...
                for xop in LEGAL_XOPS:
                    for condition in range(16):
                        emit(
                            ...
                            not_counter_expired=not_counter_expired,
                            probe=DREG((sf + xop + condition) & 0xF),
                        )
    ...
    for _ in range(random_count):
        ...
        if choice == 0:
            ...
            emit(
                ...
                not_counter_expired=bool(rng.randrange(2)),
                probe=probe,
            )
        elif choice == 16:
            emit(
                execute=True,
                opcode=_opcode(rng.randrange(16), 1, rng.randrange(16)),
                probe=probe,
            )
        elif choice == 17:
            emit(
                execute=True,
                opcode=rng.choice((0, 0x0D0000, 0x0E0010, 0x0F0000)),
                probe=probe,
            )
        elif choice == 18:
...
def main() -> int:
    ...
    parser.add_argument("--random-count", type=int, default=45_000)
    parser.add_argument("--seed", type=lambda value: int(value, 0), default=0x210016)
    args = parser.parse_args()
    args.output.parent.mkdir(parents=True, exist_ok=True)
    lines = generate_lines(args.random_count, args.seed)
    args.output.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"wrote {len(lines)} Type 16 stateful vectors to {args.output}")
    return 0
