#!/usr/bin/env python3
"""Generate deterministic Type 5/native-PM/HALT composition vectors."""
...
def _astat(state: ComputePMHaltState) -> tuple[int, int]:
    mask = 0
    value = 0
    for bit in ASTATBit:
...
def generate_lines(
    ...
    def emit(
        *,
        ...
        lines.append(f"{stimulus:055x} {events:032x} {post:047x}")
        state = result.state
    ...
    for bank_index in (0, 1):
        ...
        emit(
            phase_override=LogicalPhase.STATE_8,
            af=ExactWord(16, 0xA100 + bank_index),
        )
        emit(
            phase_override=LogicalPhase.STATE_8,
            mf=ExactWord(16, 0xB100 + bank_index),
        )
        ...
        if state.control.mode is HaltControlMode.HALTED:
            ...
            if phase is LogicalPhase.STATE_8 and advance and not held_core:
                ...
                if choice < 15:
                    ...
                    controls["af"] = ExactWord(16, rng.randrange(1 << 16))
                elif choice == 20:
                    controls["mf"] = ExactWord(16, rng.randrange(1 << 16))
                elif choice < 22:
...
def main() -> int:
    ...
    if min(coverage.values()) == 0:
        raise RuntimeError(f"insufficient Type 5/HALT coverage: {coverage}")
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text("\n".join(lines) + "\n", encoding="ascii")
    print(
        f"PASS generated {len(lines)} Type 5/native-PM/HALT clocks "
        f"seed={args.seed:#x} "
