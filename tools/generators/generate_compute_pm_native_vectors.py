#!/usr/bin/env python3
"""Generate deterministic Type 5/cache/native-PM integration vectors."""
...
def _astat(state: ComputePMNativeState) -> tuple[int, int]:
    mask = 0
    value = 0
    for bit in ASTATBit:
...
def generate_lines(random_count: int, seed: int) -> list[str]:
    ...
    for bank in (0, 1):
        ...
        emit(phase_override=LogicalPhase.STATE_8, af=ExactWord(16, 0xA100 + bank))
        emit(phase_override=LogicalPhase.STATE_8, mf=ExactWord(16, 0xB100 + bank))
...
def main() -> int:
    ...
    parser.add_argument(
        "--seed", type=lambda value: int(value, 0), default=0x2100C5
    )
    ...
    print(
        f"PASS generated {len(lines)} Type 5/native-PM clocks "
