#!/usr/bin/env python3
"""Generate Type 5/shared-PM/BR-BG differential vectors."""
...
from tools.generators.generate_compute_pm_vectors import (  # noqa: E402
...
def _astat(state: ComputePMOwnerControlState) -> tuple[int, int]:
    mask = 0
    value = 0
    for bit in ASTATBit:
...
def generate_lines(
    ...
    coverage = {name: 0 for name in (
        "type5_accept", "type5_retry", "data_complete", "recovery",
        "fetch_fill", "type13_complete", "collision", "recognized",
        "grant", "resume", "masked",
    )}
    ...
    def emit(
        *,
        ...
        type13: bool = False,
        type13_address: int = 0,
        type13_address_valid: bool = True,
        type13_write: bool = False,
        type13_data: int = 0,
        type13_data_valid: bool = True,
        execute: bool = False,
        opcode: int = 0,
        pmd: int = 0,
        pmd_valid: bool = True,
        ...
        if probe_dreg is None:
            # The bounded Type 5 engine exposes the active PM DREG read port;
            # keep the observation selector aligned with the opcode field.
        ...
        type13_request = (
            ProgramBusRequest(
                address=(ExactWord(14, type13_address)
                         if type13_address_valid else UNKNOWN),
                data_access=True,
                write=type13_write,
                write_data=(ExactWord(24, type13_data)
                            if type13_data_valid else UNKNOWN),
            )
            if type13 else None
        )
        result = apply_compute_pm_owner_control_cycle(
            state,
            reset=reset,
            phase=use_phase,
        ...
        for value, width in (
            ...
            (type13, 1), (type13_address, 14), (type13_address_valid, 1),
            (type13_write, 1), (type13_data, 24), (type13_data_valid, 1),
        ...
        for value, width in (
            ...
            (result.type5_request_presented, 1),
            (result.type5_request_accepted, 1),
        ...
        af_known, af_value = _exact(bank.af)
        mf_known, mf_value = _exact(bank.mf)
        mr_known, mr_value = _mr(bank)
        ...
        for value, width in (
            ...
            (px_known, 1), (px_value, 8), (af_known, 1), (af_value, 16),
            (mf_known, 1), (mf_value, 16), (mr_known, 1), (mr_value, 40),
        ...
        lines.append(f"{stimulus:059x} {events:029x} {post:047x}")

        for key, hit in (
            ("type5_accept", result.type5_request_accepted),
            ...
            ("type13_complete", owner.type13_completion),
    ...
    emit(phase_override=LogicalPhase.STATE_8, af=ExactWord(16, 0x1357))
    emit(phase_override=LogicalPhase.STATE_8, mf=ExactWord(16, 0x2468))
    ...
    for _ in range(random_count):
        ...
        if enabled_eight:
            ...
            if choice in (7, 20, 21, 22):
                controls.update(
                    type13=True, type13_address=rng.randrange(1 << 14),
                    type13_address_valid=rng.randrange(17) != 0,
                    type13_write=bool(rng.randrange(2)),
                    type13_data=rng.randrange(1 << 24),
                    type13_data_valid=rng.randrange(17) != 0,
                )
...
def main() -> int:
    ...
    parser.add_argument(
        "--seed", type=lambda value: int(value, 0), default=0x21A05B
    )
    ...
    print(
        f"PASS generated {len(lines)} Type 5/shared-PM/BR-BG clocks "
        f"seed={args.seed:#x} "
